...
private:
    ...
    // Що зараз стоїть у полях годинника. Заповнюється системним часом
    // при відкритті діалогу, застосовується кнопкою.
    int clk_hh_ = 0, clk_mm_ = 0, clk_dd_ = 1, clk_mon_ = 1, clk_yy_ = 26;

    // Прокрутка всього вмісту діалогу. Заголовок і "Готово" в неї не
    // входять: вони рамка, а не вміст.
    int header_count_ = 0;      // скільки перших дітей це нерухома рамка
    int content_top_ = 0;
    int content_h_ = 0;
    int scroll_ = 0;
    int scroll_max_ = 0;
    bool row_visible(int h) const;
};
