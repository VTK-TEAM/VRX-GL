...
public:
    ...
    // Копія кадру, що зараз на екрані. Копія тримає буфер живим, тож її
    // можна спокійно віддати іншому потоку — саме так і робить знімок,
    // якому на кодування треба десятки мілісекунд.
    bool snapshot(SourceFrame& out) const override;
    SourceStats stats() const override;

    // Відкрити сеанс і стати на позицію t від його початку.
    //
    // Індекс передається ЩОРАЗУ, а не зберігається всередині: у живого
    // сеансу він росте, і копія в джерелі застигла б на тому, яким запис
    // був у мить відкриття.
    ...
    bool seek(const record::SessionIndex& ix, int64_t t_us);

    // Посунути межу безпечного читання за оновленим журналом. Для живого
    // запису це і є "таймлайн доріс": дані, яких хвилину тому ще не було
    // на носії, тепер можна читати.
    void refresh(const record::SessionIndex& ix);
