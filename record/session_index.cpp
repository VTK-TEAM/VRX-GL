...
SeekPoint SessionIndex::locate(const std::string& channel, int64_t t_us) const {
    ...
    // НІЧОГО НЕ ПОКРИВАЄ ЦЮ МИТЬ — беремо початок НАСТУПНОГО шматка.
    //
    // Спокуса відповісти "немає" виглядає чесною й ламає плеєр. Канали
    // стартують не одночасно: заміряно, захват о нульовій секунді сеансу,
    // основний на 68 мс пізніше. Відкриваючи сеанс з нуля, основний діставав
    // би "немає", пайплайн не піднімався б узагалі — і канал лишався б
    // чорним НАЗАВЖДИ, а не перші 68 мс.
    //
    // Те саме стосується провалів усередині: стати на початок наступного
    // шматка безпечно, бо кадри з майбутнього все одно чекають своєї
    // секунди — джерело віддає лише те, що вже настало.
    const Entry* next = nullptr;
    for (const Entry& e : entries_) {
        if (e.f.channel != channel || e.f.anchor_us <= 0) continue;
        if (e.f.anchor_us <= want) continue;
        if (!next || e.f.anchor_us < next->f.anchor_us) next = &e;
    }
    if (next) {
        sp.name = next->f.name;
        sp.byte_off = 0;
    ...
    return sp;      // після кінця каналу — таки нічого
}
