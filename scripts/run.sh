#!/usr/bin/env bash
# VRX-GL: запуск із МАШИНИ РОЗРОБКИ, де є робочий стіл.
#
# Робить усе сам: перевіряє залежності, за потреби збирає, піднімає
# станцію й повертає робочий стіл, коли вона зупиниться.
#
# НА СТАНЦІЇ ЦЕ НЕ ВИКОРИСТОВУЄТЬСЯ. Там наглядача піднімає systemd
# (ExecStart=/root/vrx-gl/build/vrx_supervisor), а далі він сам вирішує,
# що на екрані. Раніше сюди вів двійковий запускач "ЗАПУСТИТИ VRX-GL"
# для кліку мишею — з появою автозапуску він став зайвим і прибраний.
# Викликати цей скрипт тепер лише руками: bash scripts/run.sh
#
