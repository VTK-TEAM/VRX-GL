...
public:
    ...
    // ЩО ЦЕ ЗА ОВЕРЛЕЙ — і, як наслідок, де його малювати.
    //
    //   Ui   курсор, кнопки, рамка виділення. ЛИШЕ на основному: за ТЗ
    //        редактор живе там, а на додатковому кнопки редактора немає.
    //   Osd  телеметрія. Малюється там, де її ввімкнено перемикачем —
    //        окремо на кожну роль (Scene::osd).
    enum class OverlayKind { Ui, Osd };
    ...
    void add_overlay(std::shared_ptr<Overlay> ovl, OverlayKind kind = OverlayKind::Ui);
