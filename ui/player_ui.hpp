...
public:
    ...
    // Сеанс щойно відкрито. Потрібне тому, хто підключає до OSD записану
    // телеметрію: до вибору сеансу її ще немає, і підключати нема чого.
    void set_on_opened(std::function<void(int role)> cb);

    // Натиснуто знімок. Шар не знає, куди складати файли — це знає той,
    // хто володіє носієм. Повертає кількість збережених каналів.
