...
```
ЗАПУСТИТИ VRX-GL.desktop
```

Усе інше робить `scripts/run.sh`: перевіряє залежності, за потреби збирає,
гасить графічну сесію, пише лог у `logs/` і піднімає станцію. Зупинка —
`Ctrl+C` або закрити вікно; робочий стіл повертається сам.

Ні встановлення, ні ярликів на робочому столі, ні правил `sudoers`.

Чотири речі, зроблені навмисно.

**Шляхи ніде не прибиті.** Ярлик передає скрипту власне розташування
(`%k`), корінь проєкту рахується від нього. Тому папка працює з будь-якого
місця й під будь-яким користувачем; у старому VRX шлях був вписаний у
текст, і на іншій машині це вимагало правок.
...
покладатись на нього не можна: якщо клікнути ярлик одразу після
завантаження, `lightdm` ще «запускається» — перевірка `is-active` каже
«ні», а DRM master він уже захопив. Ловилось наживо.

**Робочий стіл вертається завжди** — через `trap EXIT`, хоч по `Ctrl+C`,
хоч після падіння. Інакше в полі лишався б чорний екран без жодного
способу цим скористатись.

> Якщо пристрій виводу тримає щось стороннє, скрипт назве винуватця
> поіменно. Сам застосунок бачить лише `Device or resource busy`, з чого
> причина не читається — на цьому я спіткнувся, коли DRM тримала стара
> версія VRX.
