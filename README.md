...
Папку проєкту можна просто передати інженеру — усе інше робить один скрипт:

```bash
bash scripts/install.sh
```

Він перевірить залежності, за потреби збере, покладе на робочий стіл два
ярлики (**VRX-GL** і **VRX-GL — зупинити**) і дозволить запускати їх без
пароля. Далі потрібна лише мишка.

| скрипт | що робить |
|---|---|
| `install.sh` | одноразове налаштування: залежності, збірка, ярлики, `sudoers` |
| `start.sh` | гасить попередню копію, пише лог у `logs/`, запускає |
| `stop.sh` | зупиняє по `SIGTERM` і **повертає робочий стіл** |

Три речі, зроблені навмисно.

**Шляхи ніде не прибиті.** Корінь проєкту рахується від розташування самого
скрипта, користувач робочого столу — з `SUDO_USER`. Тому папка працює з
будь-якого місця й під будь-яким іменем; у старому VRX шляхи були вписані
в текст, і на іншій машині це вимагало правок.
...
іншого каталогу дав би відео без OSD.

**`stop.sh` вертає `lightdm`.** Застосунок сам зупиняє графічну сесію (DRM
master ексклюзивний), тож після виходу екран лишився б чорним — в полі це
означало б, що інженеру нема чим користуватись.

> Запуск без пароля тримається на правилі в `/etc/sudoers.d/vrx-gl`, а
> скрипти лежать у домашньому каталозі. Отже той, хто має доступ до цього
> користувача, може їх змінити й дістати root. Для наземної станції це
> прийнятно, для машини загального користування — ні.
...
display/
  ...
  kms_display_manager.*     DRM/KMS atomic, гаряча заміна монітора
render/
...
| потік | що робить |
|---|---|
| рендерер | зводить шари, комітить у KMS |
