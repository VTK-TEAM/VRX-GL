...
public:
    struct Config {
        ...
        // НАСКІЛЬКИ СТИСКАТИ ТЕЛЕМЕТРІЮ В РЕЖИМІ ПЛЕЄРА. Унизу екрана там
        // таймлайн і кнопки, і нижній ряд опинявся б під ними. Пропорції
        // при цьому не зберігаються — свідомо: підписи трохи вужчають, і це
        // заважає менше, ніж затулений рядок.
        float playback_squeeze = 0.93f;
    ...
    // Показувати на цій ролі ЗАПИСАНУ телеметрію замість ефірної.
    // nullptr повертає до ефіру. Малювання від цього не міняється взагалі:
    // підміняється лише джерело значень.
    void set_playback_storage(int role, VtTelemetryStorage* s);
